...
## Deployment — auto-packaged Cloudflare Pages bundle

Building locally auto-packages a Cloudflare Pages bundle to `~/Desktop/{slug}-cloudflare-pages/` (folder + matching `.zip`). The slug is derived from `business.config.json → businessName`.

Useful for:
- **(a)** deploying to Cloudflare Pages instead of Vercel — drag the folder or .zip onto Cloudflare Pages → Direct Upload
- **(b)** sending client demo zips without exposing the GitHub repo
...
**Supported trade keys** (canonical):
`softwash`, `roofing`, `lawn-care`, `fencing`, `auto-detailing`, `junk-removal`, `hvac`, `plumbing`, `electrical`, `painting`, `tree-services`, `cleaning`, `salon`, `barber`, `beauty`, `fitness`, `mover`, `pet`, `general`.

Aliases also work: `painter` → painting, `electrician` → electrical, `plumber` → plumbing, `roofer` → roofing, `landscaper` → lawn-care, `spa` → beauty, `gym` → fitness, `groomer` → pet, `contractor` → general, etc.

Anything else → obvious gray "Add Photo" placeholder cards from `placehold.co` (NOT random unrelated stock photos — a hair salon site that accidentally shows a swimming pool is the bug this fixed). `scripts/sync-business-info.mjs` also prints a warning if `business.config.json → tradeType` doesn't match any known key.
