...
### Admin auth
Uses `ADMIN_PASSWORD` env var (set by Render Blueprint, auto-generated). The dashboard:
- Prompts for the password on first visit at `/admin`
- Sends it as `Authorization: Bearer <password>` on every protected request
- Stores it in `localStorage` so you don't re-enter every page load
- Click "Sign out" to clear localStorage

Endpoints:
- `POST /api/admin/login` — validates the password, returns `{ ok: true }`
...
- `SESSION_SECRET` — auto-generated by render.yaml (used by lib/auth.ts for JWT signing; the legacy `JWT_SECRET` env var still works for backward compat)
